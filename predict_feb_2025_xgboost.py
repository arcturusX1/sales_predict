...
# Filter training data to January 2025
train = daily[(daily['year'] == 2025) & (daily['month'] == 1)]
...
# Save the model
model.save_model("xgboost_jan_2025.json")

# Now, predict February 2025
# February 2025 has 28 days (not leap year)
feb_dates = pd.date_range(start='2025-02-01', end='2025-02-28')

# Get unique outlets that have Jan 2025 data
jan_outlets = daily[(daily['year'] == 2025) & (daily['month'] == 1)]['Outlet'].unique()

predictions = []

for outlet in jan_outlets:
    # Get the last date in Jan 2025 for this outlet
    outlet_jan = daily[(daily['Outlet'] == outlet) & (daily['year'] == 2025) & (daily['month'] == 1)]
    if outlet_jan.empty:
        continue
    last_date = outlet_jan['Date'].max()
    ...
    # Get the last row
    last_row = outlet_jan[outlet_jan['Date'] == last_date].iloc[0].copy()
    
    # Initialize current_row
    current_row = last_row.copy()
    
    # Adjust feb_dates to start from last_date + 1
...
print("Predicted Monthly Sales for February 2025 (XGBoost):")
print(monthly_sales)

# Save predictions
pred_df.to_csv('february_2025_predictions_xgboost.csv', index=False)
monthly_sales.to_csv('february_2025_monthly_sales_xgboost.csv', index=False)

# Plot the chart
plt.figure(figsize=(14, 8))
plt.bar(monthly_sales['Outlet'], monthly_sales['Predicted February 2025 Sales'], color='lightgreen')
plt.xticks(rotation=45, ha='right')
plt.xlabel('Outlet')
plt.ylabel('Predicted February 2025 Sales')
plt.title('Predicted February 2025 Sales per Outlet (XGBoost)')
plt.tight_layout()
plt.savefig('february_2025_sales_chart_xgboost.png', dpi=300, bbox_inches='tight')
plt.show()